    ...
    <main style={{ background: "#0C0D14", minHeight: "100vh" }}>
      ...
      {activeTab !== "Global" ? (
        ...
        <section className="px-6 max-w-6xl mx-auto pb-16">
          <div className="rounded-xl p-10 text-center" style={{ background: "#1A1B2E", border: "1px solid #2A2B3D" }}>
            ...
            <p className="text-sm mb-6" style={{ color: "#A0A3B1" }}>
              Be the first to challenge someone and start climbing!
            </p>
            <Link
              href="/login"
        ...
        <>
              ...
              <div className="grid grid-cols-3 gap-3">
                {players.slice(0, 3).map((player) => (
                  ...
                  >
                    ...
                    <div
                      className="w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center mx-auto mb-2 text-white font-bold text-xs md:text-sm"
                      style={{ background: "linear-gradient(135deg, #3865FF, #7B2CFF)" }}
                    >
                      {player.initials}
                    </div>
                    <p className="font-semibold text-white text-xs md:text-sm truncate">{player.name}</p>
                    <p className="text-lg md:text-2xl font-bold mt-1" style={{ color: "#3865FF" }}>
                      {player.elo}
                    </p>
                    <p className="text-xs mt-1" style={{ color: "#6B6E80" }}>
          ...
          {/* MOBILE: cards */}
          <section className="px-6 pb-16 max-w-6xl mx-auto md:hidden">
            <div className="flex flex-col gap-3">
              {players.map((player) => (
                ...
                >
                  {/* Rank */}
                  <span className="text-sm font-bold w-8 text-center flex-shrink-0" style={{ color: "#A0A3B1" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs flex-shrink-0" style={{ color: "#6B6E80" }}>
                        {player.winRate}%
                      </span>
                    </div>
                  </div>
                  ...
                  {/* ELO */}
                  <span className="text-sm font-bold flex-shrink-0" style={{ color: "#3865FF" }}>
                    {player.elo}
                  </span>

                  {/* Challenge */}
                  <Link
                    href={`/challange/${player.userId}`}
          ...
          {/* DESKTOP: table */}
          <section className="px-6 pb-16 max-w-6xl mx-auto hidden md:block">
                  ...
                  {players.map((player, i) => (
                    <tr
                      ...
                      style={{
                        ...
                        background: i % 2 === 0 ? "#1A1B2E" : "#13141F",
                      }}
                    >
                      ...
                      <td className="px-4 py-3 text-sm font-bold" style={{ color: "#3865FF" }}>
                        {player.elo}
                      </td>
                      <td className="px-4 py-3 text-sm" style={{ color: "#A0A3B1" }}>
                        {player.wins}
                      </td>
                      <td className="px-4 py-3 text-sm" style={{ color: "#A0A3B1" }}>
                        {player.losses}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs" style={{ color: "#A0A3B1" }}>
                            {player.winRate}%
                          </span>
                        </div>
                      </td>
